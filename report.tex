...
From the dense path samples $\{\mathbf{p}_j\}_{j=0}^{M-1}$, curvature $\kappa_j$ is estimated numerically. Curvature estimation can be noisy if computed via second derivatives of noisy coordinates; this matters because the speed limit scales with $1/\sqrt{|\kappa|}$. We therefore either (a) re-sample at constant arc-length step $\Delta s$ and apply mild smoothing, or (b) smooth the offset function before generating the dense path (Sec.~\ref{sec:regularization}). These steps reduce curvature spikes without changing the qualitative outcome of the study.
...
\begin{equation}
T = \sum_{j=0}^{M-2} \frac{\Delta s_j}{\max(v_{\mathrm{mid},j},\,v_{\min})},
\label{eq:laptime}
\end{equation}
where $\Delta s_j$ is the segment length between consecutive dense samples and $v_{\mathrm{mid},j}$ is the midpoint speed for that segment. Using true geometric $\Delta s_j$ avoids bias from assuming constant spacing, and the small floor $v_{\min}$ prevents division by zero if the profile momentarily touches zero.
...
To address waypoint-induced curvature peaks, we include an optional smoothness term:
\begin{equation}
f(\mathbf{d}) = T(\mathbf{d}) + \lambda \, \Omega(\mathbf{d}),
\end{equation}
where $\Omega(\mathbf{d})$ penalizes discrete curvature of the offset sequence, e.g.,
\begin{equation}
\Omega(\mathbf{d}) = \sum_{i=0}^{N-1} \bigl(d_{(i+1)\bmod N}-2d_i+d_{(i-1)\bmod N}\bigr)^2,
\end{equation}
which uses wrap-around indexing for the closed loop. Intuitively, this discourages the GA from producing sharp local changes between neighboring waypoints that a spline must interpolate exactly.

An alternative (sometimes stronger) fix is to replace interpolating splines with \emph{smoothing splines} for $d(s)$, so that the final trajectory is not forced to pass exactly through all waypoints; this directly reduces curvature spikes at knots. Both approaches are consistent with the goal of producing physically interpretable trajectories under a curvature-sensitive speed model.

\subsection{Selection, variation, and elitism}
We use a standard real-coded GA with:
\begin{itemize}
    \item Tournament selection.
    \item Crossover between parent offset vectors (e.g., arithmetic crossover or SBX \cite{b_deb1995}).
...
A core experimental control is a fixed evaluation budget $B$ (number of fitness evaluations). For a population size $P$ and $G$ generations, a typical budget is $B \approx P\times G$ (plus initialization). Using a fixed budget ensures fair comparison across friction settings and repeated runs, since each run consumes the same computational effort.
...
\subsection{Measured outcome (example results)}
Using our implementation and diagnostics, we obtained the following summary (example from one set of experiments):
...
\hline
0.60 & 38.01 & 0.60 & 37.14\\
0.90 & 31.87 & 0.90 & 30.29\\
1.20 & 27.60 & 0.57 & 26.53\\
\hline
\end{tabular}
\label{tab:lapstats}
\end{center}
...
\end{equation}
Lateral acceleration is also most consistent when evaluated at segment midpoints:
\begin{equation}
a_{\mathrm{lat},j} \approx v_{\mathrm{mid},j}^2 |\kappa_{\mathrm{mid},j}|.
\end{equation}
...
\begin{itemize}
    \item \textbf{Apparent friction-circle violations:} When $a_{\mathrm{lat}}$ and $a_{\mathrm{long}}$ are computed with mismatched discretizations (pointwise vs segmentwise) or with assumed constant $\Delta s$, a small subset of points may appear slightly outside the friction circle. Using true segment lengths and midpoint-consistent accelerations removes most of these artifacts.
    \item \textbf{Waypoint-aligned spikes:} Interpolating splines forced through independently optimized waypoints can create curvature peaks at knot locations. This produces sharp speed-limit changes and large reconstructed accelerations. Smoothing-spline reconstruction or explicit smoothness regularization mitigates this issue.
\end{itemize}
